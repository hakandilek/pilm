...
/*
 if ( !supportsvg.support && supportsvg.builtin )
	alert( "You need to enable SVG support first!" );
else if ( !supportsvg.support )
	alert( "You need a browser with SVG support" );
else if ( supportsvg.native == "Opera" && supportsvg.builtinVersion < 9 )
	alert( "Opera 8 is not compatible with this program.. please upgrade Opera to at least version 9" );
else if ( supportsvg.support == "Plugin" && !supportsvg.IID )
	alert( "Please disable the " + supportsvg.plugin + " SVG plugin as it is not compatible with this software for this browser" );
else	alert( "Success! Starting the application.." );
*/
//alert(supportsvg.support);
//alert(supportsvg.builtin);
